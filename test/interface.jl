for T in (Float32, Float64, BigFloat)
    @testset "interface" begin
        @testset "single-valued" begin
            ...
            # Test hvp!
            v = T.([0.111, -1234])
            @test hvp!(td, x_seed, v) == h_x_seed * v

            # Test that the call counter is not incremented:
            # `hvp!` falls back to computing Hessian, but the Hessian at `x_seed` is cached
            @test h_calls(td) == 1
            @test iszero(hvp_calls(td))
            ...
            @testset "Hessian-vector product" begin
                for (name, prob) in MultivariateProblems.UnconstrainedProblems.examples
                    if prob.istwicedifferentiable
                        ...
                        xT = T.(prob.initial_x)
                        nxT = length(xT)
                        ddf = TwiceDifferentiable(only_fg_and_hvp!(fg!, _hvp!), xT)
                        x = rand(xT, nxT)
                        v = rand(xT, nxT)
                        G = NLSolversBase.alloc_DF(x, T(0))
                        H = NLSolversBase.alloc_H(x, T(0))
                        MVP.hessian(prob)(H, x)
                        @test hvp!(ddf, x, v) == H*v
                        @test ddf.HVP == H*v
                        F = fg!(0.0, G, x)
                        ...
                        @test g_calls(ddf) == 1
                        @test h_calls(ddf) == 0
                        @test hvp_calls(ddf) == 1
                        clear!(ddf)
                        @test f_calls(ddf) == 0
                        @test g_calls(ddf) == 0
                        @test h_calls(ddf) == 0
                        @test hvp_calls(ddf) == 0
                    end
                end
            end
        end
