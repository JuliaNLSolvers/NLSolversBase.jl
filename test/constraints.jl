@testset "Constraints" begin
    @testset "ConstraintBounds" begin
        ...
        cb = ConstraintBounds([1, 2], [3, 4.0], [], [10.0, 20.0, 30.0])
        io = IOBuffer()
        show(io, cb)
        s = String(take!(io))
    ...
    @testset "Once differentiable constraints" begin
        ...
        odc = OnceDifferentiableConstraints(
            cbd.c!, cbd.jacobian!,
        ...
        @testset "autodiff" begin
            ...
            @test_throws ArgumentError OnceDifferentiableConstraints(
                cbd.c!, cbd.lx, cbd.ux,
                cbd.lc, cbd.uc
            )
            ...
            @test_throws ArgumentError("The autodiff value `:wuoah` is not supported. Use `:finite` or `:forward`.") OnceDifferentiableConstraints(
                cbd.c!, lx, ux,
                lc, uc, :wuoah
            )
            for autodiff in (:finite, :forward)
                odca = OnceDifferentiableConstraints(
                    cbd.c!, lx, ux,
                    lc, uc, autodiff
                )
                ...
                @test isapprox(Jarr, Jarra, atol = 1.0e-10)
            end
        end
    end
    ...
    @testset "Twice differentiable constraints" begin
        ...
        odc = TwiceDifferentiableConstraints(
            cbd.c!, cbd.jacobian!, cbd.h!,
            cbd.lx, cbd.ux, cbd.lc, cbd.uc
        )
        @test isempty(odc.bounds.bx)
        @test isempty(odc.bounds.bc)
        ...
        @testset "second order autodiff" begin
            ...
            for autodiff in (:finite, :forward) #testing double differentiation
                ...
                @test isapprox(jac_result, jac_result_autodiff, atol = 1.0e-10)
                @test isapprox(hess_result, hess_result_autodiff, atol = 1.0e-6)
