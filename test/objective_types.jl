for T in (Float32, Float64, BigFloat)
    @testset "objective types" begin
        ...
        @testset "no fg!" begin
            Random.seed!(324)
            od =
                OnceDifferentiable(exponential, exponential_gradient!, x_seed, T(0), g_seed)
            xrand = rand(T, 2)
            ...
            value_gradient!(od, zeros(T, 2))
            gradient!(od, xrand)
            @test value(od, zeros(T, 2)) == od.F
            @test value(od, zeros(T, 2)) == value(od)
            ...
            value_gradient!(td, zeros(T, 2))
            gradient!(td, xrand)
            @test value(td, zeros(T, 2)) == td.F
            @test value(td, zeros(T, 2)) == value(td)
