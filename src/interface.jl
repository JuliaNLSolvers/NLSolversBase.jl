...
"""
    hvp!(obj::TwiceDifferentiable, x::AbstractArray, v::AbstractArray)
...
!!! note
    This function does use cached results if available.
"""
function hvp!(obj::TwiceDifferentiable, x, v)
    if x != obj.x_hvp || v != obj.v_hvp
        hvp!!(obj, x, v)
    end
    return obj.HVP
end
...
function hvp!!(obj::TwiceDifferentiable, x, v)
    if obj.hv !== nothing
        ...
        hessian!(obj, x)
        LinearAlgebra.mul!(obj.HVP, obj.H, v)
    end
    copyto!(obj.x_hvp, x)
    copyto!(obj.v_hvp, v)
    return obj.HVP
...
function _clear_hvp!(d::AbstractObjective)
    d.hvp_calls = 0
    fill!(d.HVP, NaN)
    fill!(d.x_hvp, NaN)
    fill!(d.v_hvp, NaN)
...
hvp_calls(d::Union{NonDifferentiable, OnceDifferentiable}) = 0
hvp_calls(d::TwiceDifferentiable) = d.hvp_calls
